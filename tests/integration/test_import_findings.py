...
mock_url_except_api_scan = re.compile(dojo_url + "/api/v2/(?!product_api_scan_configurations).*")
...
class TestImportFindingsFromFile:
    ...
    def test_import_existing_test_findings(self):
        response = json.dumps({"count": 1, "results": [{"id": 1, "name": "Test Item"}]}).encode()
    ...
    def test_import_new_test_findings(self):
        ...
        responses.add(responses.GET, dojo_url + "/api/v2/test_types/", body=response, status=200)
        responses.add(responses.GET, mock_url_except_test_type, body=get_response, status=200)
...
class TestApiScanImport:
    ...
    def test_import_with_existing_configuration(self):
        response = json.dumps({"count": 1, "results": [{"id": 1, "name": "Test Item"}]}).encode()
        get_response = json.dumps({"count": 0, "results": []}).encode()
        create_response = json.dumps({"id": 1, "name": "API Scan Configuration"}).encode()
        responses.add(responses.POST, mock_url, body=create_response, status=201)
        responses.add(
            responses.GET,
        ...
        responses.add(responses.GET, mock_url_except_api_scan, body=response, status=200)
        responses.add(responses.POST, dojo_url + "/api/v2/reimport-scan/", status=200)
        main()
